...
bool LeggedController::init(hardware_interface::RobotHW* robot_hw, ros::NodeHandle& controller_nh) {
  ...
  #ifdef TEST
  sub_ = nh.subscribe("end_effector_pos", 1, &LeggedController::endEffectorPositionsCallback, this);
  #endif
...
#ifdef TEST
void LeggedController::endEffectorPositionsCallback(const geometry_msgs::PointStampedConstPtr& msg) {
  // Update the end effector class attribute with the new end-effector positions.
  effector_position_(0) = msg->point.x;
  effector_position_(1) = msg->point.y;
  effector_position_(2) = msg->point.z;
 // ROS_INFO_STREAM("ROS EE POSITION RECEIVED");
...
void LeggedController::update(const ros::Time& time, const ros::Duration& period) {
  ...
  #ifdef WBC_CONTROL
  vector_t x = wbc_->update(optimizedState, optimizedInput, measuredRbdState, plannedMode, period.toSec(), effector_position_);
  #else
