...
vector_t WeightedWbc::update(const vector_t& stateDesired, const vector_t& inputDesired, const vector_t& rbdStateMeasured, size_t mode,
  ...
  // Cost
  Task weighedTask = formulateWeightedTasks(stateDesired, inputDesired, period, effector_position);
...
#ifdef WBC_CONTROL
Task WeightedWbc::formulateWeightedTasks(const vector_t& stateDesired, const vector_t& inputDesired, scalar_t period,const vector3_t& effector_position) {
  return formulateSwingLegTask(effector_position) * weightSwingLeg_ + formulateBaseAccelTask(stateDesired, inputDesired, period) * weightBaseAccel_ +
