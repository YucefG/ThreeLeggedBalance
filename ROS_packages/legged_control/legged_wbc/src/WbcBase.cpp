...
#ifdef WBC_CONTROL
Task WbcBase::formulateSwingLegTask(const vector3_t& effector_position) {
  ...
  // set pinoc interface for desired feet trajectories
  ...
  //no velocity when point reached
  velDesired[0].setZero();

  //posDesired is in odom's frame, as effector position
  posDesired[0] = effector_position ;
